...
class AdminController extends Controller
{
    ...
    public function invoice_submit(request $request){
        ...
        // return sizeof();
        // $empty_array = array();
        // if( sizeof($request->attraction_or_activity) == 0 ){
        ...
        $this->validate($request, [
            'attraction_or_activity' => 'required',
        ]);
        
        $product = [];
        foreach($request->attraction_or_activity as $row){
            if (empty($value)){
                return redirect()->back()->with('warning','Please Add Within 1 Product');
            }else{
