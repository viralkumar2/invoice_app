               ...
               <div class="col-12 table-responsive">
                  <form action="{{ url('store-input-fields') }}" method="POST">
                     ...
                     <table class="table table-bordered" id="dynamicAddRemove">
                         ...
                         <tr>
                             <td>
                              <select class="form-control product"name="attraction_or_activity[]" id="product_1" data-id='1' onchange='product_chnage()'>
                             ...
                             <td><input type="text" class="form-control add_category" id="add_category1"/></td>
   ...
   //  var $wrapper = $('.multi-fields', this);
      ...
      // var select = '<select class="text-two'+x+'" name="attraction_or_activity[]"><option value="activity">Activity</option></select>';
         ...
         $("#dynamic-ar").click(function () {
               ...
               // $('.item_select').html('');
               $("#dynamicAddRemove").append('<tr><td><select class="text-two form-control product item_select" id=product_'+x+' data-id='+x+' name="attraction_or_activity"><option >Select Product</option></select></td><td><input type="text" class="form-control add_category" id="add_category'+x+'"/></td><td><input type="number" readonly class="form-control add_price" id=add_price/></td><td><input type="number" class="form-control" value="1"/></td><td>1.1</td><td><button type="button" class="btn btn-outline-danger remove-input-field">Delete</button></td></tr>'
               );

               $.each(products, function( index, value ) {
                  
                  // if(selectedItem_1 != value['id']){
                     $('#product'+x+'').append('<option value='+value['id']+'>'+value['name']+'</option>');
                  // }
                  // else{
                  //    $('.item_select').append('<option selected value='+value['id']+'>'+value['name']+'</option>');
                  // }
               });
      ...
      function product_chnage(){
         ...
         alert($('#product1 option:selected').data('data-id'));
         console.log( $("#product1 option:selected").data("id"));
